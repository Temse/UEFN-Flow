cat << 'INNER_EOF' > backup_import.txt
...
sed -i -e '/const confirmDeleteProject = async () => {/,/if (fileInputRef.current) fileInputRef.current.value = "";\n  };/c\
'"$(cat backup_import.txt | sed 's/$/\\/')"'' src/components/Dashboard.tsx
